...
(defun %generate (name hash-fn test-fn valuep store-hash optimize)
  (check-type name symbol)
  (check-type hash-fn function-designator)
  (check-type test-fn function-designator)
  (let ((%make   (symbolicate '#:%make- name))
    ...
    `(macrolet ((eref (p s) `(aref ,p (the fixnum (1+ (the fixnum (ash ,s 1)))))) ; probe entry-index+1
       ...
       (defstruct (,name (:constructor ,%make) (:copier nil) (:predicate nil))
         ...
         (kv (lose) :type simple-vector)
         (imask 15 :type u32)          ; capacity (size of probe, power of two) - 1
         (count 0 :type u32)           ; live entries
         (hwm 0 :type u32)             ; append frontier (in entries)
         (ilimit 11 :type u32)         ; grow threshold = load-factor * cap
         (load-factor 0.7)
         (kv-size 1.5)
         ,@(when store-hash               ; parallel per-entry hash32 for compaction
       ...
       (defun ,grow (table)
         (let* ((pr (,probe table)) (ok (,kv table))
           ...
           (declare (type (simple-array u32 (*)) pr np) (type simple-vector ok nk)
                    (type u32 cap ncap nlim nklim hwm nimask))
...
(defmacro define-hash-table (name hash-fn test-fn &key store-hash (optimize '(speed)))
...
workload. :OPTIMIZE specifies optimization declaration."
  (%generate name hash-fn test-fn t store-hash optimize))

(defmacro define-hash-set (name hash-fn test-fn &key store-hash (optimize '(speed)))
...
of recomputing HASH-FN, use it for an expensive HASH-FN with delete-heavy
workload. :OPTIMIZE specifies optimization declaration."
  (%generate name hash-fn test-fn nil store-hash optimize))
